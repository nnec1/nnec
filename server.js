...
app.post("/api/collect-fee", authenticate, async (req, res) => {
  ...
  try {
    ...
    await db.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      student_id, paymentAmount, finalTotalFee, newPaidFee, finalRemaining,
      paymentDate, finalDueDate, issueDate, receipt_number, notes || null
    ]);
...
app.get("/api/dashboard-stats", authenticate, async (req, res) => {
  try {
    ...
    const [debtors] = await db.execute(`
      SELECT COUNT(DISTINCT fp.student_id) as total 
      FROM fee_payments fp
      JOIN students s ON fp.student_id = s.id
...
app.get("/api/recent-transactions", authenticate, async (req, res) => {
  ...
  try {
    const [results] = await db.execute(`
      SELECT 
        fp.id, fp.amount, fp.payment_date, fp.receipt_number, fp.issue_date,
...
app.get("/api/financial-summary", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (period === "daily") {
      const today = new Date().toISOString().split("T")[0];
      const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE DATE(issue_date) = ?`, [today]);
      total_income = income[0]?.total || 0;
    } else if (period === "monthly") {
      const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE MONTH(issue_date) = MONTH(CURDATE()) AND YEAR(issue_date) = YEAR(CURDATE())`);
      total_income = income[0]?.total || 0;
    } else if (period === "yearly") {
      const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE YEAR(issue_date) = YEAR(CURDATE())`);
      total_income = income[0]?.total || 0;
    } else if (start_date && end_date) {
      const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE DATE(issue_date) BETWEEN ? AND ?`, [start_date, end_date]);
      total_income = income[0]?.total || 0;
    }
    res.json({ total_income, total_expense: 0, net_profit: total_income, transaction_count: 0 });
  } catch (err) {
...
app.post("/api/employees", authenticate, upload.single("photo"), async (req, res) => {
  ...
  try {
    const [result] = await db.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')
    `, [name, toNull(father_name), toNull(phone), email, hashedPass, position, toNull(salary), toNull(hire_date), toNull(photoPath)]);
...
app.get("/api/issue-dates", authenticate, async (req, res) => {
  try {
    const [results] = await db.execute(`SELECT DISTINCT DATE(issue_date) as issue_date FROM fee_payments WHERE issue_date IS NOT NULL ORDER BY issue_date DESC`);
...
app.get("/api/daily-fee-stats-with-expiry", authenticate, async (req, res) => {
  ...
  try {
    const [payments] = await db.execute(`
      SELECT 
        fp.id, fp.student_id, fp.amount, fp.total_fee, fp.paid_fee, fp.remaining_after,
        fp.payment_date, fp.issue_date, fp.due_date, fp.receipt_number, fp.notes,
        s.name as student_name, s.father_name, s.student_card_id, c.class_name
      FROM fee_payments fp
      JOIN students s ON fp.student_id = s.id
      JOIN classes c ON s.class_id = c.id
    ...
    res.json({ success: true, date: targetDate, total_amount: totalToday, student_count: uniqueStudents, transaction_count: formattedPayments.length, payments: formattedPayments });
  } catch (err) {
  ...
  try {
    if (period === "daily") {
      for (let i = 6; i >= 0; i--) {
        ...
        const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE DATE(issue_date) = ?`, [dateStr]);
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const [income] = await db.execute(`SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE DATE(issue_date) BETWEEN ? AND ?`, [startDate, endDate]);
        incomes.push(income[0]?.total || 0);
        expenses.push(0);
      }
    } else if (period === "yearly") {
    ...
    res.json({ periods, incomes, expenses, total_income, total_expense, net_profit: total_income - total_expense });
  } catch (err) {
...
app.post("/api/complaints", authenticate, async (req, res) => {
  ...
  try {
    const [result] = await db.execute(`INSERT INTO complaints (student_id, subject, message, status, created_at) VALUES (?, ?, ?, 'pending', NOW())`, [student_id, subject, message]);
...
app.put("/api/complaints/:id", authenticate, async (req, res) => {
  ...
  try {
    await db.execute(`UPDATE complaints SET response = ?, status = 'resolved', resolved_at = NOW() WHERE id = ?`, [response, req.params.id]);
    res.json({ success: true });
  } catch (err) {
