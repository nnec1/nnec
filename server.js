...
const isCEO = (req, res, next) => {
    if (req.user.role !== "ceo") return res.status(403).json({ error: "دسترسی محدود به ریس سیستم" });
    next();
};

const isAdminOrCEO = (req, res, next) => {
    if (!["ceo", "admin"].includes(req.user.role)) return res.status(403).json({ error: "دسترسی محدود به مدیران" });
    next();
};
...
app.post("/api/login", async (req, res) => {
    ...
    try {
        ...
        let redirectUrl =
            user.position === "ceo" ? "/ceo/dashboard.html" :
            user.position === "admin" ? "/admin/dashboard.html" :
            "/teacher/dashboard.html";

        res.json({ success: true, redirectUrl, user: { id: user.id, name: user.name, role: user.position } });
    } catch (err) {
...
app.post("/api/student-login-with-qr", async (req, res) => {
    ...
    try {
        ...
        res.cookie("token", token, { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 });
        res.json({ success: true, redirectUrl: "/student/dashboard.html", user: { id: user.id, name: user.name } });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "خطای سرور" });
    }
...
app.get("/api/classes", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT * FROM classes WHERE is_active = 1");
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/classes/:id", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT * FROM classes WHERE id = ?", [req.params.id]);
        res.json(results[0]);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/classes", authenticate, isAdminOrCEO, async (req, res) => {
    ...
    try {
        const [result] = await db.execute(
            ...
            [class_name, start_time || "08:00:00", is_active !== undefined ? is_active : 1]
        );
...
app.put("/api/classes/:id", authenticate, isAdminOrCEO, async (req, res) => {
    ...
    try {
        await db.execute(
            ...
            [class_name, start_time || "08:00:00", is_active !== undefined ? is_active : 1, req.params.id]
        );
...
app.get("/api/admins", authenticate, isCEO, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT id, name, father_name, phone, email, status, created_at FROM employees WHERE position = 'admin' ORDER BY created_at DESC");
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/teachers", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT id, name, email, phone FROM employees WHERE position = 'teacher' AND status = 'active'");
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/employees", authenticate, upload.single("photo"), async (req, res) => {
    const { name, father_name, phone, email, password, position, salary, hire_date } = req.body;

    if (position === "admin" && req.user.role !== "ceo") {
        return res.status(403).json({ error: "❌ فقط ریس سیستم می‌تواند مدیر ایجاد کند" });
    }
    if ((position === "teacher" || position === "accountant") && !["ceo", "admin"].includes(req.user.role)) {
        return res.status(403).json({ error: "❌ فقط مدیر یا ریس می‌توانند استاد و حسابدار ایجاد کنند" });
    }
    ...
    try {
        const [result] = await db.execute(
            `INSERT INTO employees (name, father_name, phone, email, password, position, salary, hire_date, photo, status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
            [name, father_name || null, phone || null, email, hashedPass, position, salary || null, hire_date || null, photoPath]
        );
        res.json({ id: result.insertId, message: `${position} با موفقیت ایجاد شد` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

app.put("/api/employees/:id", authenticate, upload.single("photo"), async (req, res) => {
    const { name, father_name, phone, email, password, position, salary, hire_date, status } = req.body;
    ...
    let values = [name, father_name || null, phone || null, email, position, salary || null, hire_date || null, status];
...
app.post("/api/students", authenticate, upload.single("photo"), async (req, res) => {
    const { name, father_name, phone, class_id, total_fee, paid_fee, due_date, address, status } = req.body;
    ...
    try {
        const [result] = await db.execute(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, CURDATE(), ?, ?, ?, ?, ?, ?, ?, ?)
        `, [student_card_id, name, father_name || null, phone || null, hashedPass, class_id, 
            status || "active", qr_token, finalTotalFee, finalPaidFee, finalRemainingFee, finalDueDate, address || null, photoPath]);
        res.json({ id: result.insertId, qr_token, student_card_id, password: autoPass });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

app.put("/api/students/:id", authenticate, upload.single("photo"), async (req, res) => {
    const { name, father_name, phone, class_id, total_fee, paid_fee, due_date, status, address, password } = req.body;
    ...
    let values = [name, father_name || null, phone || null, class_id, finalTotalFee, finalPaidFee, finalRemainingFee, finalDueDate, status, address || null];
...
app.get("/api/teacher/info/:teacherId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT id, name, father_name, phone, email, photo FROM employees WHERE id = ?", [req.params.teacherId]);
        res.json(results[0] || {});
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/teacher/save-attendance", authenticate, async (req, res) => {
    ...
    try {
        const [existing] = await db.execute(`SELECT id FROM daily_attendance WHERE teacher_id = ? AND class_id = ? AND attendance_date = ?`, [teacher_id, class_id, date]);
        ...
        const [result] = await db.execute(`INSERT INTO daily_attendance (teacher_id, class_id, attendance_date) VALUES (?, ?, ?)`, [teacher_id, class_id, date]);
        const attId = result.insertId;
        for (const a of attendance) {
            await db.execute(`INSERT INTO attendance_details (attendance_id, student_id, status, notes) VALUES (?, ?, ?, ?)`, [attId, a.student_id, a.status, a.notes || null]);
        }
...
app.get("/api/student/fees/:studentId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT total_fee, paid_fee, remaining_fee, due_date FROM students WHERE id = ?`, [req.params.studentId]);
...
app.get("/api/student/payments/:studentId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT * FROM fee_payments WHERE student_id = ? ORDER BY payment_date DESC`, [req.params.studentId]);
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/collect-fee", authenticate, async (req, res) => {
    ...
    try {
        const [student] = await db.execute(`SELECT s.*, c.class_name FROM students s JOIN classes c ON s.class_id = c.id WHERE s.id = ?`, [student_id]);
        ...
        await db.execute(`UPDATE students SET paid_fee = ?, remaining_fee = ?, due_date = ? WHERE id = ?`, [newPaidFee, finalRemainingFee, finalDueDate, student_id]);
        await db.execute(`INSERT INTO fee_payments (student_id, amount, payment_date, receipt_number, notes) VALUES (?, ?, ?, ?, ?)`, [student_id, paymentAmount, paymentDate, receipt_number, notes || null]);
        res.json({
            success: true,
...
app.get("/api/fee-payments-history", authenticate, async (req, res) => {
    ...
    let params = [];
    if (start_date && end_date) { query += ` AND fp.payment_date BETWEEN ? AND ?`; params.push(start_date, end_date); }
...
app.delete("/api/fee-payment/:id", authenticate, async (req, res) => {
    ...
    try {
        const [payment] = await db.execute(`SELECT amount FROM fee_payments WHERE id = ?`, [paymentId]);
        if (payment.length === 0) return res.status(404).json({ error: "پرداخت یافت نشد" });
        const paymentAmount = parseFloat(payment[0].amount);
        await db.execute("DELETE FROM fee_payments WHERE id = ?", [paymentId]);
        const [student] = await db.execute(`SELECT total_fee, paid_fee FROM students WHERE id = ?`, [student_id]);
        const newPaidFee = (parseFloat(student[0].paid_fee) || 0) - paymentAmount;
        const newRemainingFee = (parseFloat(student[0].total_fee) || 0) - newPaidFee;
        await db.execute(`UPDATE students SET paid_fee = ?, remaining_fee = ? WHERE id = ?`, [newPaidFee < 0 ? 0 : newPaidFee, newRemainingFee < 0 ? 0 : newRemainingFee, student_id]);
        res.json({ success: true });
    } catch (err) {
...
app.put("/api/fee-payment/:id", authenticate, async (req, res) => {
    ...
    if (isNaN(newAmount) || newAmount < 0) return res.status(400).json({ error: "مبلغ معتبر وارد کنید" });
    try {
        const [oldPayment] = await db.execute(`SELECT amount FROM fee_payments WHERE id = ?`, [paymentId]);
        if (oldPayment.length === 0) return res.status(404).json({ error: "پرداخت یافت نشد" });
        const oldAmount = parseFloat(oldPayment[0].amount);
        const amountDiff = newAmount - oldAmount;
        await db.execute(`UPDATE fee_payments SET amount = ?, payment_date = ?, notes = ? WHERE id = ?`, [newAmount, payment_date, notes || null, paymentId]);
        const [student] = await db.execute(`SELECT total_fee, paid_fee FROM students WHERE id = ?`, [student_id]);
...
app.get("/api/announcements", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT * FROM announcements WHERE is_active = 1 AND (expires_at IS NULL OR expires_at >= CURDATE()) ORDER BY created_at DESC`);
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/announcements", authenticate, upload.single("file"), async (req, res) => {
    const { title, content, target, expires_at } = req.body;
    const filePath = req.file ? `/uploads/${req.file.filename}` : null;
    try {
        const [result] = await db.execute(`INSERT INTO announcements (title, content, target, file_path, expires_at, created_by) VALUES (?, ?, ?, ?, ?, ?)`, [title, content, target, filePath, expires_at || null, req.user.id]);
...
app.get("/api/complaints", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT c.*, s.name as student_name, e.name as teacher_name FROM complaints c LEFT JOIN students s ON c.student_id = s.id LEFT JOIN employees e ON c.teacher_id = e.id ORDER BY c.created_at DESC`);
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post("/api/complaints", authenticate, async (req, res) => {
    ...
    try {
        const [result] = await db.execute(`INSERT INTO complaints (student_id, subject, message) VALUES (?, ?, ?)`, [student_id || null, subject, message]);
        res.json({ id: result.insertId });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.put("/api/complaints/:id", authenticate, async (req, res) => {
    ...
    try {
        await db.execute(`UPDATE complaints SET status='resolved', response=?, resolved_at=NOW() WHERE id=?`, [response, req.params.id]);
...
app.get("/api/student/info/:studentId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT s.*, c.class_name FROM students s LEFT JOIN classes c ON s.class_id = c.id WHERE s.id = ?`, [req.params.studentId]);
        res.json(results[0]);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/student/grades/:studentId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT g.*, sub.subject_name FROM grades g JOIN subjects sub ON g.subject_id = sub.id WHERE g.student_id = ? ORDER BY g.exam_date DESC`, [req.params.studentId]);
        res.json(results);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/student/attendance/:studentId", authenticate, async (req, res) => {
    ...
    try {
        const [details] = await db.execute(`SELECT ad.status, ad.notes, da.attendance_date as date FROM attendance_details ad JOIN daily_attendance da ON ad.attendance_id = da.id WHERE ad.student_id = ? AND MONTH(da.attendance_date)=? AND YEAR(da.attendance_date)=?`, [req.params.studentId, month, year]);
        res.json({ details });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.put("/api/student/update-profile/:studentId", authenticate, upload.single("photo"), async (req, res) => {
    ...
    if (photoPath) { setClause += `, photo=?`; values.push(photoPath); }
    if (password && password.trim()) { const hashed = await bcrypt.hash(password, 10); setClause += `, password=?`; values.push(hashed); }
...
app.get("/api/settings", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT setting_key, setting_value FROM system_settings");
        const settings = {};
...
app.post("/api/change-password", authenticate, async (req, res) => {
    ...
    try {
        const [results] = await db.execute(`SELECT password FROM ${table} WHERE id = ?`, [req.user.id]);
        if (results.length === 0) return res.status(500).json({ error: "کاربر یافت نشد" });
        let isValid = false;
        if (results[0].password.startsWith("$2a$")) isValid = await bcrypt.compare(current, results[0].password);
        else isValid = current === results[0].password;
        if (!isValid) return res.status(401).json({ error: "رمز فعلی اشتباه است" });
        const hashed = await bcrypt.hash(newPassword, 10);
        await db.execute(`UPDATE ${table} SET password = ? WHERE id = ?`, [hashed, req.user.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get("/api/dashboard-stats", authenticate, async (req, res) => {
    try {
        if (req.user.role === "admin") {
            const [students] = await db.execute(`SELECT COUNT(*) as total_students FROM students WHERE status='active'`);
            const [teachers] = await db.execute(`SELECT COUNT(*) as total_teachers FROM employees WHERE position='teacher'`);
            const [debtors] = await db.execute(`SELECT COUNT(*) as total_debtors FROM students WHERE remaining_fee > 0`);
            const [revenue] = await db.execute(`SELECT COALESCE(SUM(amount),0) as monthly_revenue FROM fee_payments WHERE MONTH(payment_date)=MONTH(CURDATE())`);
            res.json({ total_students: students[0]?.total_students || 0, total_teachers: teachers[0]?.total_teachers || 0, total_debtors: debtors[0]?.total_debtors || 0, monthly_revenue: revenue[0]?.monthly_revenue || 0 });
        } else { res.json({}); }
...
app.get("/api/ceo/dashboard-stats", authenticate, isCEO, async (req, res) => {
    try {
        const [admins] = await db.execute(`SELECT COUNT(*) as total_admins FROM employees WHERE position='admin'`);
        const [teachers] = await db.execute(`SELECT COUNT(*) as total_teachers FROM employees WHERE position='teacher'`);
        const [students] = await db.execute(`SELECT COUNT(*) as total_students FROM students WHERE status='active'`);
        const [income] = await db.execute(`SELECT COALESCE(SUM(amount),0) as yearly_income FROM fee_payments WHERE YEAR(payment_date)=YEAR(CURDATE())`);
        res.json({ total_admins: admins[0]?.total_admins || 0, total_teachers: teachers[0]?.total_teachers || 0, total_students: students[0]?.total_students || 0, yearly_income: income[0]?.yearly_income || 0 });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/active-classes", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT c.id, c.class_name, c.start_time, (SELECT COUNT(*) FROM teacher_classes tc WHERE tc.class_id = c.id) as teacher_count, (SELECT COUNT(*) FROM students s WHERE s.class_id = c.id AND s.status = 'active') as student_count FROM classes c WHERE c.is_active = 1 ORDER BY c.class_name`);
        res.json(results);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/fee-defaulters", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT s.*, c.class_name FROM students s JOIN classes c ON s.class_id = c.id WHERE s.remaining_fee > 0 AND s.status = 'active' ORDER BY s.remaining_fee DESC`);
        res.json(results);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/fee-expired", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT s.*, c.class_name FROM students s JOIN classes c ON s.class_id = c.id WHERE s.due_date < CURDATE() AND s.remaining_fee > 0 AND s.status = 'active' ORDER BY s.due_date ASC`);
        res.json(results);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/class-subjects/:classId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT s.*, tc.subject_id FROM subjects s JOIN teacher_classes tc ON s.id = tc.subject_id WHERE tc.class_id = ?`, [req.params.classId]);
        res.json(results);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/teacher/timetable/:teacherId", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute(`SELECT t.*, c.class_name, s.subject_name FROM timetable t JOIN classes c ON t.class_id = c.id JOIN subjects s ON t.subject_id = s.id WHERE t.teacher_id = ? ORDER BY FIELD(t.day_of_week, 'saturday', 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday'), t.start_time`, [req.params.teacherId]);
        res.json(results);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/homework/:id", authenticate, async (req, res) => {
    try {
        const [results] = await db.execute("SELECT * FROM homework WHERE id = ?", [req.params.id]);
        res.json(results[0]);
    } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post("/api/homework", authenticate, async (req, res) => {
    const { class_id, subject_id, teacher_id, homework_date, due_date, assignment } = req.body;
    try {
        const [result] = await db.execute(`INSERT INTO homework (class_id, subject_id, teacher_id, homework_date, due_date, assignment) VALUES (?, ?, ?, ?, ?, ?)`, [class_id, subject_id, teacher_id, homework_date, due_date, assignment]);
        res.json({ id: result.insertId });
    } catch (err) { res.status(500).json({ error: err.message }); }
});

app.put("/api/homework/:id", authenticate, async (req, res) => {
    const { class_id, subject_id, homework_date, due_date, assignment } = req.body;
    try {
        await db.execute(`UPDATE homework SET class_id=?, subject_id=?, homework_date=?, due_date=?, assignment=? WHERE id=?`, [class_id, subject_id, homework_date, due_date, assignment, req.params.id]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/student/stats/:studentId", authenticate, async (req, res) => {
    try {
        const [presentCount] = await db.execute(`SELECT COUNT(*) as count FROM attendance_details ad JOIN daily_attendance da ON ad.attendance_id = da.id WHERE ad.student_id = ? AND ad.status = 'present' AND YEAR(da.attendance_date) = YEAR(CURDATE())`, [req.params.studentId]);
        const [absentCount] = await db.execute(`SELECT COUNT(*) as count FROM attendance_details ad JOIN daily_attendance da ON ad.attendance_id = da.id WHERE ad.student_id = ? AND ad.status = 'absent' AND YEAR(da.attendance_date) = YEAR(CURDATE())`, [req.params.studentId]);
        const [lateCount] = await db.execute(`SELECT COUNT(*) as count FROM attendance_details ad JOIN daily_attendance da ON ad.attendance_id = da.id WHERE ad.student_id = ? AND ad.status = 'late' AND YEAR(da.attendance_date) = YEAR(CURDATE())`, [req.params.studentId]);
        const [grades] = await db.execute(`SELECT AVG((score/max_score)*100) as avg_grade FROM grades WHERE student_id = ?`, [req.params.studentId]);
        res.json({ present_count: presentCount[0]?.count || 0, absent_count: absentCount[0]?.count || 0, late_count: lateCount[0]?.count || 0, avg_grade: Math.round(grades[0]?.avg_grade || 0) });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/report/teacher-attendance", authenticate, async (req, res) => {
    ...
    try {
        const [teachers] = await db.execute(`SELECT id, name, phone FROM employees WHERE position = 'teacher' AND status = 'active'`);
        const [taken] = await db.execute(`SELECT DISTINCT teacher_id FROM daily_attendance WHERE attendance_date = ?`, [targetDate]);
        const takenIds = taken.map(t => t.teacher_id);
        const notTaken = teachers.filter(t => !takenIds.includes(t.id));
        res.json({ date: targetDate, not_taken: notTaken, total_teachers: teachers.length });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get("/api/report/student-attendance", authenticate, async (req, res) => {
    ...
    let query = `SELECT ad.*, s.id as student_id, s.name, s.father_name, s.student_card_id, da.attendance_date, c.class_name, c.id as class_id FROM attendance_details ad JOIN daily_attendance da ON ad.attendance_id = da.id JOIN students s ON ad.student_id = s.id JOIN classes c ON s.class_id = c.id WHERE da.attendance_date BETWEEN ? AND ?`;
    let params = [start_date, end_date];
    if (class_id && class_id !== "") { query += ` AND s.class_id = ?`; params.push(class_id); }
    if (student_name && student_name !== "") { query += ` AND s.name LIKE ?`; params.push(`%${student_name}%`); }
