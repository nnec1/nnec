...
app.get("/api/fee-debtors", authenticate, async (req, res) => {
  try {
    ...
    console.log("✅ Debtors found (latest records):", results.length);
    res.json(results);
  } catch (err) {
...
// ====================== منقضی شده (آخرین رکورد هر شاگرد) ======================
app.get("/api/fee-expired", authenticate, async (req, res) => {
  try {
    ...
    console.log("✅ Expired found (latest records):", results.length);
    res.json(results);
  } catch (err) {
...
app.get("/api/financial-summary", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (period === "daily") {
      ...
      const [income] = await db.execute(
        `SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE MONTH(issue_date) = MONTH(CURDATE()) AND YEAR(issue_date) = YEAR(CURDATE())`,
      );
      total_income = income[0]?.total || 0;
    } else if (period === "yearly") {
      const [income] = await db.execute(
        `SELECT COALESCE(SUM(amount), 0) as total FROM fee_payments WHERE YEAR(issue_date) = YEAR(CURDATE())`,
      );
      total_income = income[0]?.total || 0;
...
app.get("/api/issue-dates", authenticate, async (req, res) => {
  try {
    const [results] = await db.execute(
      `SELECT DISTINCT DATE(issue_date) as issue_date FROM fee_payments WHERE issue_date IS NOT NULL ORDER BY issue_date DESC`,
    );
...
app.post("/api/complaints", authenticate, async (req, res) => {
  ...
  try {
    const [result] = await db.execute(
      `INSERT INTO complaints (student_id, subject, message, status, created_at) VALUES (?, ?, ?, 'pending', NOW())`,
